...
class ClassificationLoss(torch.nn.Module):
    ...
    def forward(self, logits, target,
        ...
        if use_hierar:
            ...
            return self.criterion(logits, target) + \
                    hierar_penalty * self.cal_recursive_regularize(hierar_paras, hierar_relations, device)
        else:
            return self.criterion(logits, target)
    ...
    def cal_recursive_regularize(self, paras, hierar_relations, device="cpu"):
        ...
        for i in range(len(paras)):
            ...
            parent_para = torch.index_select(paras, 0, torch.tensor(i).to(device))
