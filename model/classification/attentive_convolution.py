...
class AttentiveConvNet(Classifier):
    def __init__(self, dataset, config):
        ...
        if self.attention_type == AttentionType.DOT:
            ...
            self.bilinear_matrix = init_tensor(
                torch.empty(self.attention_dim, self.attention_dim)).to(config.device)
            self.dot_product_attention = DotProductAttention(1.0)
        ...
        self.linear = torch.nn.Linear(self.dim + 2*self.hidden_size, len(dataset.label_map))
    ...
    def forward(self, batch):
        ...
        hidden = torch.nn.functional.max_pool1d(
            attentive_convolution, kernel_size=attentive_convolution.size()[-1]).squeeze()
