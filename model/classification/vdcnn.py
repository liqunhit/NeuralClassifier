...
class VDCNN(Classifier):
    ...
    def __init__(self, dataset, config):
        ...
        """
        super(VDCNN, self).__init__(dataset, config)

        self.vdcnn_num_convs = dict()
        ...
        self.vdcnn_depth = config.VDCNN.vdcnn_depth
        ...
        self.top_k = self.config.VDCNN.top_k_max_pooling
        ...
        self.linear3 = torch.nn.Linear(2048, len(dataset.label_map))
        self.dropout = torch.nn.Dropout(p=config.train.hidden_layer_dropout)

    def get_parameter_optimizer_dict(self):
        params = super(VDCNN, self).get_parameter_optimizer_dict()
    ...
    def forward(self, batch):
        ...
        for i in range(0, len(self.num_kernels)):
            ...
            if i < len(self.num_kernels) - 1:
                ...
                pool = F.max_pool1d(conv_block, kernel_size=3, stride=2)
            else:
                # k-max-pooling
        ...
        return self.dropout(self.linear3(fc2))
